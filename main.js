...
function createWindow() {
  ...
  mainWindow = new BrowserWindow({
    ...
    trafficLightPosition: { x: 20 + MARGIN, y: 16 + MARGIN },
...
ipcMain.on('sb:open', () => {
  sbIsOpen = true;
  layoutUIExpanded(); // フルサイズに展開
  bringUIToFront();   // uiViewを前面に
...
ipcMain.on('sb:close', () => {
  ...
  layoutUI(); // uiViewを16px幅に縮小
  // WebViewを前面に → uiViewの16px帯はWebViewの後ろに
  if (activeTabId) {
    const v = webViews.get(activeTabId);
    if (v) try { mainWindow.setTopBrowserView(v); } catch(e) {}
  }
  // uiViewを再度前面に（16px幅なのでWebViewと重ならない）
  bringUIToFront();
  mainWindow.setWindowButtonVisibility(false);
});
