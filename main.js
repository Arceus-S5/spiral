...
const SB_W = 240; // サイドバー幅（index.htmlの--sb-wと合わせる）
...
function activateWebView(view) { layoutWebView(view); }
function layoutWebViewWithSB(view) { layoutWebView(view); }

function layoutUI() {
  const [w, h] = getWH();
  uiView.setBounds({ x: -9999, y: 0, width: w, height: h });
}
...
function bringWebToFront() { bringWebViewToFront(); }
...
function createWindow() {
  ...
  mainWindow = new BrowserWindow({
    width: 900 + MARGIN * 2, height: 680 + MARGIN * 2,
  ...
  // シャドウ専用ビュー（常に最背面）
  shadowView = new BrowserView({ backgroundColor: '#00000000', webPreferences: { nodeIntegration: false } });
  mainWindow.addBrowserView(shadowView);
  const [sw, sh] = getWH();
  const EX = 20;
  shadowView.setBounds({ x: MARGIN - EX, y: MARGIN - EX, width: sw - MARGIN * 2 + EX * 2, height: sh - MARGIN * 2 + EX * 2 });
  shadowView.webContents.loadFile('src/shadow.html');
  // shadowViewを最背面に（uiViewより後ろ）
  try { mainWindow.setTopBrowserView(uiView); } catch(e) {}
  layoutUI();
  ...
  mainWindow.on('resize', () => {
    ...
    if (shadowView) { const EX = 20; shadowView.setBounds({ x: MARGIN - EX, y: MARGIN - EX, width: w - MARGIN * 2 + EX * 2, height: h - MARGIN * 2 + EX * 2 }); }
    bringUIToFront();
  });
...
ipcMain.handle('tab:create', async (event, payload) => {
  ...
  const ROUNDED_CSS = `
    ...
    * { overscroll-behavior: none !important; }
    ...
    body { border-radius: 10px !important; overflow: hidden !important; background: transparent !important; }
  `;
...
ipcMain.on('theme:set', (event, theme) => {
  if (!mainWindow || mainWindow.isDestroyed()) return;
  mainWindow.setBackgroundColor(theme === 'dark' ? '#1a1a24' : '#ededf3');
});
