...
function createWindow() {
  ...
  mainWindow.webContents.session.on('will-download', (event, item) => {
    ...
    item.once('done', (e, state) => {
      ...
      entry.savedPath = item.getSavePath();
      entry.completedAt = Date.now();
      saveDownloadHistory();
      if (uiView && !uiView.webContents.isDestroyed()) {
        uiView.webContents.send('download:done', entry);
      }
    });
  });
...
ipcMain.handle('tab:create', async (event, url) => {
  ...
  view.webContents.on('did-navigate', (e, u) => {
    ...
    // 履歴に追加（about:blank等は除く）
    if (u && u.startsWith('http')) {
      addHistory({ url: u, title: view.webContents.getTitle() || u });
    }
  });
  ...
  view.webContents.on('page-title-updated', (e, t) => {
    ...
    // 履歴の最新エントリのタイトルを更新
    if (view._url && view._url.startsWith('http') && browseHistory.length > 0 && browseHistory[0].url === view._url) {
      browseHistory[0].title = t;
      saveHistory();
    }
    // 通知検知
...
function loadHistory() {
  try {
    if (fs.existsSync(historyPath)) {
      browseHistory = JSON.parse(fs.readFileSync(historyPath, 'utf8'));
    }
...
// ── スペースデータ ──
const spacesDataPath = path.join(app.getPath('userData'), 'spaces-data.json');

function loadSpacesData() {
  try {
    if (fs.existsSync(spacesDataPath)) {
      return JSON.parse(fs.readFileSync(spacesDataPath, 'utf8'));
    }
  } catch(e) {}
  return null;
}

function saveSpacesData(data) {
  try { fs.writeFileSync(spacesDataPath, JSON.stringify(data, null, 2)); } catch(e) {}
}

ipcMain.handle('spaces:load', () => loadSpacesData());
ipcMain.handle('spaces:save', (e, data) => { saveSpacesData(data); return true; });
