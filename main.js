...
// ── V8: ヒープはほどよく（メモリ節約 + GCが暴走しない範囲）──
app.commandLine.appendSwitch('js-flags', [
  '--max-old-space-size=1024',
  '--max-semi-space-size=64',
  '--expose-gc',                     // タブ閉じ時に手動GCを呼べるようにする
].join(' '));

// ── メモリ節約 ──
app.commandLine.appendSwitch('memory-pressure-off');                 // OSからのメモリ圧迫通知を無視して安定させる
app.commandLine.appendSwitch('process-per-site');                    // 同一サイトのタブはプロセスを共有（メモリ削減）
app.commandLine.appendSwitch('renderer-process-limit', '6');         // レンダラープロセス上限を適切に設定

// ── ページ表示速度 ──
app.commandLine.appendSwitch('enable-features', [
  'NetworkServiceInProcess2',        // ネットワークをメインプロセスで処理（IPC削減）
  'ParallelDownloading',             // リソース並列ダウンロード
  'PrefetchDNS',                     // DNSプリフェッチ
].join(','));
...
// ── キャッシュ（ページ再表示を高速化） ──
app.commandLine.appendSwitch('disk-cache-size', String(256 * 1024 * 1024));  // 512→256MB
app.commandLine.appendSwitch('media-cache-size', String(64 * 1024 * 1024));  // 128→64MB
...
// UIのダーク/ライトはCSS data-theme属性で管理。ChromeはOSに従う。
...
let sbIsOpen = false; // サイドバー開閉状態


// 余白（ドロップシャドウ用マージン）
const MARGIN = 20;
const MARGIN_BOTTOM = 20; // 上下左右すべて同じ20px
...
function bringWebViewToFront() {
  if (!activeTabId) return;
  const v = webViews.get(activeTabId);
  if (v) try { mainWindow.setTopBrowserView(v); } catch(e) {}
  layoutUI();
  bringUIToFront();
}
...
function sendNativeNotification({ title, body, tabId, appKey }) {
  // UIの通知ハブにも追加
  if (uiView && !uiView.webContents.isDestroyed()) {
    uiView.webContents.send('notif:hub', { title, body, tabId, appKey, time: Date.now() });
  }
...
function createBackgroundView(appKey, appConf) {
  ...
  const view = new BrowserView({
    webPreferences: {
      ...
      backgroundThrottling: true,    // 通知監視はスロットリングON（タイトル変化は検知できる）
      userAgent: UA,
...
function createWindow() {
  ...
  // シャドウ専用ビュー（最背面に追加してからuiViewを追加）
  shadowView = new BrowserView({ backgroundColor: '#00000000', webPreferences: { nodeIntegration: true, contextIsolation: false } });
  mainWindow.addBrowserView(shadowView);
  const [sw, sh] = getWH();
  const EX = 20;
  shadowView.setBounds({ x: 0, y: 0, width: sw, height: sh });
  ...
  // uiViewはデフォルトセッションを使う → Google Suggest CORS許可のため設定
  ...
  mainWindow.on('resize', () => {
    ...
    if (shadowView) { shadowView.setBounds({ x: 0, y: 0, width: w, height: h }); }
    bringUIToFront();
  });
  ...
  // DevTools ショートカット (Cmd+Option+I または F12)
  ...
  uiView.webContents.on('before-input-event', (event, input) => {
    if (input.type === 'keyDown' && input.key === 'F12') toggleDevTools();
    if (input.type === 'keyDown' && input.key === 'i' && input.meta && input.alt) toggleDevTools();
  });
...
function setupSession(ses) {
  ...
  ses.webRequest.onBeforeSendHeaders((details, callback) => {
    ...
    // brotli圧縮を要求 → 転送量削減で高速化
    if (!headers['Accept-Encoding']) headers['Accept-Encoding'] = 'gzip, deflate, br, zstd';
  ...
  ses.webRequest.onHeadersReceived((details, callback) => {
    ...
    delete headers['X-Frame-Options'];
    // Google Suggest APIのCORSを許可（uiViewから直接fetchするため）
    if (details.url && details.url.includes('suggestqueries.google.com')) {
...
ipcMain.handle('tab:create', async (event, payload) => {
  ...
  view.webContents.on('did-finish-load', () => {
    view.webContents.insertCSS(ROUNDED_CSS).catch(() => {});
  });
  view.webContents.on('did-navigate-in-page', () => {
    view.webContents.insertCSS(ROUNDED_CSS).catch(() => {});
  });
  // dom-ready でも早めに注入（角丸を初期レンダリングに間に合わせる。color-schemeは注入しない → OSに従う）
  ...
  view.webContents.on('context-menu', (e, params) => {
    ...
    // ── ナビゲーション ──
    items.push({ label: '戻る',   enabled: view.webContents.canGoBack(),    click: () => view.webContents.goBack() });
    items.push({ label: '進む',   enabled: view.webContents.canGoForward(), click: () => view.webContents.goForward() });
    items.push({ label: '再読み込み', click: () => view.webContents.reload() });
    items.push({ type: 'separator' });

    // ── リンク ──
    if (params.linkURL) {
      items.push({ label: 'リンクを新しいタブで開く', click: () => uiView.webContents.send('app:openUrl', params.linkURL) });
      items.push({ label: 'リンクのURLをコピー',      click: () => require('electron').clipboard.writeText(params.linkURL) });
      items.push({ type: 'separator' });
    }

    // ── 画像 ──
    if (params.hasImageContents) {
      items.push({ label: '画像を新しいタブで開く', click: () => uiView.webContents.send('app:openUrl', params.srcURL) });
      items.push({ label: '画像のURLをコピー',      click: () => require('electron').clipboard.writeText(params.srcURL) });
      items.push({ label: '画像を保存',             click: () => view.webContents.downloadURL(params.srcURL) });
      items.push({ type: 'separator' });
    }
    ...
    if (params.selectionText) {
      ...
      items.push({
        label: `"${params.selectionText.slice(0, 20)}${params.selectionText.length > 20 ? '…' : ''}" をGoogle検索`,
        click: () => uiView.webContents.send('app:openUrl', `https://www.google.com/search?q=${encodeURIComponent(params.selectionText)}`),
      });
      items.push({ type: 'separator' });
    }
    ...
    if (params.isEditable) {
      items.push({ label: '切り取り', role: 'cut',   enabled: params.editFlags.canCut });
      items.push({ label: 'コピー',   role: 'copy',  enabled: params.editFlags.canCopy });
      items.push({ label: '貼り付け', role: 'paste', enabled: params.editFlags.canPaste });
      items.push({ type: 'separator' });
    }

    // ── ページ ──
    items.push({ label: 'ページのURLをコピー', click: () => require('electron').clipboard.writeText(view.webContents.getURL()) });
    items.push({ label: '名前を付けて保存',    click: () => view.webContents.savePage(
      require('path').join(require('os').homedir(), 'Downloads', (view.webContents.getTitle() || 'page') + '.html'),
      'HTMLComplete'
    ).catch(() => {}) });
    items.push({ type: 'separator' });
    items.push({ label: 'デベロッパーツール', click: () => view.webContents.openDevTools({ mode: 'detach' }) });
...
ipcMain.handle('tab:activate', async (event, id) => {
  for (const [vid, v] of webViews) {
    ...
    if (vid !== id) {
      ...
      // バックグラウンドに回ったらスロットリングON（CPU・メモリ節約）
      try { v.webContents.setBackgroundThrottling(true); } catch(e) {}
    }
  }
...
ipcMain.handle('ext:uninstall', async (event, extId) => {
  try {
    ...
    for (const ses of sessions) {
      try {
        const loaded = ses.getAllExtensions().find(e => e.id === extId);
        if (loaded) await ses.removeExtension(extId);
      } catch(e) { console.log('removeExtension error:', e.message); }
    }
    ...
    console.log('ext:uninstall error:', e.message);
    return { error: e.message };
  }
});
...
// テーマ変更 → shadowViewの背景色を切り替え
ipcMain.on('theme:set', (e, dark) => {
...
ipcMain.on('trig:enter', () => {
  if (!sbIsOpen) {
    ...
    bringUIToFront();
    if (uiView && !uiView.webContents.isDestroyed())
      uiView.webContents.send('trig:enter');
  }
});
ipcMain.on('trig:leave', () => {
  if (!sbIsOpen) {
    layoutUI();
    if (uiView && !uiView.webContents.isDestroyed())
      uiView.webContents.send('trig:leave');
  }
});
