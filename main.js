...
function setupAutoUpdater() {
  ...
  const CHROME_VERSION = '136.0.7103.92';

  // electron-updater は checkForUpdates() 内で app.getVersion() を呼び出し
  // Semver として検証する。app.setVersion(CHROME_VERSION) で偽装すると
  // 4セグメントのChromeバージョンが渡されてエラーになるため、
  // checkForUpdates 呼び出し前に正規バージョンへ一時的に戻し、
  // 完了後に Chrome バージョンへ再設定する。
  async function safeCheckForUpdates(updater) {
    app.setVersion(APP_VERSION);
    try {
      return await updater.checkForUpdates();
    } finally {
      try { app.setVersion(CHROME_VERSION); } catch {}
    }
  }
  ...
  try {
    ...
    setTimeout(() => safeCheckForUpdates(autoUpdater).catch(e => {
    ...
    ipcMain.handle('update:check', async () => {
      try {
        const result = await safeCheckForUpdates(autoUpdater);
