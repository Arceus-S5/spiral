...
function showWsLoginModal(wsIdx) {
  ...
  document.getElementById('ws-login-go-btn').addEventListener('click', async () => {
    // モーダルをログイン中表示に切り替え（閉じない）
    const modal = document.getElementById('ws-login-modal');
    modal.innerHTML = `
      <div style="font-size:13px;font-weight:600;color:var(--text);">ログイン中...</div>
      <div style="font-size:11px;color:var(--text2);line-height:1.6;">Googleアカウントでログインしてください<br>ログイン完了後、自動でワークスペースが使えます</div>
      ...
      <button id="ws-login-cancel-btn" style="background:transparent;border:none;color:var(--text2);font-size:12px;cursor:pointer;font-family:inherit;padding:4px;">キャンセル</button>
    `;
