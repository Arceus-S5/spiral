...
function switchSettingsPane(pane) {
  ...
  document.getElementById('settings-pane-title').textContent =
    pane === 'general' ? '一般' : pane === 'network' ? 'ネットワーク' : pane === 'download' ? 'ダウンロード' : pane === 'mail' ? 'メールアドレス' : pane === 'phone' ? '電話番号' : '詳細設定';
...
window.browser.onSettingsApplied && window.browser.onSettingsApplied(state => {
  ...
  if (state.dblclickClose !== undefined) S.dblclickClose = state.dblclickClose;
...
window.browser.onReady(async () => {
  ...
  if (saved) {
    ...
    if (saved.dblclickClose !== undefined) {
      ...
      document.getElementById('setting-dblclick-close').checked = S.dblclickClose;
    }
    if (saved.keyboardScroll !== undefined) {
      S.keyboardScroll = saved.keyboardScroll;
      document.getElementById('setting-keyboard-scroll').checked = S.keyboardScroll;
    }
