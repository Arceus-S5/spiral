...
function renderBMNode(node, container) {
  if (node.type === 'folder') {
    ...
    arrowSpan.className = 'folder-icon';
    arrowSpan.innerHTML = ARROW_SVG;
    ...
    arrowSpan.addEventListener('click', e => { e.stopPropagation(); toggleFolder(); });
    nameSpan.addEventListener('click', e => { e.stopPropagation(); toggleFolder(); });
    head.addEventListener('click', e => {
      if (e.target === dragH || dragH.contains(e.target)) return;
      toggleFolder();
    });
...
function renderTabNode(node, container) {
  if (node.type === 'folder') {
    ...
    arrowSpan2.className = 'folder-icon';
    arrowSpan2.innerHTML = ARROW_SVG;

    const nameSpan2 = document.createElement('span');
    nameSpan2.className = 'tab-title';
    nameSpan2.textContent = node.name;

    const cntSpan = document.createElement('span');
    cntSpan.style.cssText = 'font-size:10px;color:var(--textm);margin-right:4px';
    cntSpan.textContent = count;

    head.appendChild(dragH2);
    head.appendChild(arrowSpan2);
    head.appendChild(nameSpan2);
    head.appendChild(cntSpan);

    function toggleTabFolder() {
      node._open = node._open === false ? true : false;
      wrap.classList.toggle('open', node._open !== false);
    }
    arrowSpan2.addEventListener('click', e => { e.stopPropagation(); toggleTabFolder(); });
    nameSpan2.addEventListener('click', e => { e.stopPropagation(); toggleTabFolder(); });
    head.addEventListener('click', e => {
      if (e.target === dragH2 || dragH2.contains(e.target)) return;
      toggleTabFolder();
    });
    head.addEventListener('contextmenu', e => {
      e.preventDefault();
...
window.browser.onUpdateAvailable(({ version }) => {
  ...
  const pw = document.getElementById('update-progress-wrap');
