...
// 保存
function saveSpaces() {
  window.browser.saveSpaces({ spaces: S.spaces });
}
...
function buildSpacePanes() {
  ...
  S.spaces.forEach((sp, i) => {
    ...
    let pressTimer = null;
    dot.addEventListener('mousedown', () => { pressTimer = setTimeout(() => openSpaceEditModal(i), 600); });
  ...
  S.spaces.forEach((sp, i) => {
    ...
    const pane = buildSpacePane(sp, i);
