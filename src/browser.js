...
const webShadow = document.getElementById('web-shadow');
const SB_W = 240;
...
function openSB() {
  ...
  if (webShadow) webShadow.style.left = SB_W + 12 + 'px';
}
function closeSB(delay = 280) {
  ...
  hideT = setTimeout(() => {
    ...
    if (webShadow) webShadow.style.left = '12px';
  }, delay);
}
...
(function() {
  ...
  function applyWidth(w) {
    ...
    if (w < DEFAULT_W) {
      ...
      sbEl.style.width = DEFAULT_W + 'px';
      sbEl.style.transformOrigin = 'top left';
      sbEl.style.transform = `scale(${scale})`;
      // scaleで高さが縮む分を逆補正
      sbEl.style.height = `calc((100% - 60px) / ${scale})`;
    } else {
      document.documentElement.style.setProperty('--sb-w', w + 'px');
...
document.getElementById('nb-theme').addEventListener('click', () => {
  ...
  window.browser.setTheme(S.dark ? 'dark' : 'light');
  scheduleSave();
});
...
// バーウィンドウからのサイドバー開閉
if (window.browser.onBarSbOpen) {
  window.browser.onBarSbOpen(() => {
    clearTimeout(hideT);
    openSB();
  });
}
if (window.browser.onBarSbClose) {
  window.browser.onBarSbClose(() => closeSB(300));
}
