...
// bm-list の空白部分右クリック → フォルダ作成
document.getElementById('bm-list').addEventListener('contextmenu', e => {
  if (e.target.closest('.bm-item, .folder-item')) return; // 既存アイテム上はスキップ
  e.preventDefault();
  showContextMenu(e.clientX, e.clientY, [
...
// tabs-list の空白部分右クリック → フォルダ作成
document.getElementById('tabs-list').addEventListener('contextmenu', e => {
  if (e.target.closest('.tab-item, .tab-folder-item')) return;
  e.preventDefault();
  showContextMenu(e.clientX, e.clientY, [
